...
- **Core / CLI:** Shared address book **`public_0x`** field, relaxed contact validation (email or wallet), top-level **`skillware addressbook`** commands (`list`, `edit`, `set-wallet`, `remove`, `open`), **`skillware config open`**, and **`skillware evm token add`** for operator token registry (#373, #374).

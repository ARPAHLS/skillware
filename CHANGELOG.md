...
- **Docs:** Structured hub architecture for catalog discovery — root README category index, [docs/sitemap.md](docs/sitemap.md), and keyword landing pages at `docs/skills/<category>/README.md` with catalog pages moved beside them (`docs/skills/<category>/<skill_name>.md`). Runtime `skills/<category>/<skill_name>` IDs and `SkillLoader.load_skill()` are unchanged (#370).
- **GitHub:** Discussion form templates under [`.github/DISCUSSION_TEMPLATE/`](.github/DISCUSSION_TEMPLATE/) for skill requests, show and tell, and integration Q&A. Enabling Discussions on the repository remains a maintainer GitHub setting (#370).

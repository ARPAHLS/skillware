...
| `gemini_uk_companies_house_handler.py` | `finance/uk_companies_house_handler` | Gemini | `[gemini]` | `GOOGLE_API_KEY`, `COMPANIES_HOUSE_API_KEY`. Demo: `UK_COMPANIES_HOUSE_EXAMPLE_DEMO=1`. | Resolve company, officers, filings via Gemini tool loop (or demo mode with mocked HTTP). |

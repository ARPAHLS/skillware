...
| `secret_provider_demo.py` | `finance/wallet_screening` | Local execute | `[finance_wallet_screening]` | None | Host-injects `ETHERSCAN_API_KEY` via `MappingSecretProvider` and `config=` without setting `os.environ`. |
...
| `context_optimizer_demo.py` | `optimization/context_optimizer` | Local execute | `[optimization_context_optimizer]` | None | Query-aware extractive selection from bundled sample policy (fastembed; first run may download model). |
| `context_optimizer_chain_demo.py` | `security/prompt_injection_firewall`, `optimization/context_optimizer` | Local execute | `[security_prompt_injection_firewall]`, `[optimization_context_optimizer]` | None | Trust-boundary chain: firewall then optimizer before a hypothetical main LLM call. |
| `context_optimizer_gemini_loop.py` | `optimization/context_optimizer` | Gemini (optional) | `[optimization_context_optimizer]`, `[gemini]` | Optional `GOOGLE_API_KEY` and `CONTEXT_OPTIMIZER_GEMINI_LIVE=1` | Optimizer selection then Gemini answer constrained to `optimized_context`. |
| `context_optimizer_claude_loop.py` | `optimization/context_optimizer` | Claude (optional) | `[optimization_context_optimizer]`, `[claude]` | Optional `ANTHROPIC_API_KEY` and `CONTEXT_OPTIMIZER_CLAUDE_LIVE=1` | Optimizer selection then Claude answer constrained to `optimized_context`. |
...
| `kpi_gate_demo.py` | `monitoring/kpi_gate` | Local execute | `[monitoring_kpi_gate]` | None | Offline KPI gate demo: weekly snapshot vs policy charter and benchmark doctrine, honest insufficient_data, and fail-closed contract errors. |
| `gemini_uk_companies_house_handler.py` | `finance/uk_companies_house_handler` | Gemini | `[finance_uk_companies_house_handler]`, `[gemini]` | `GOOGLE_API_KEY`, `COMPANIES_HOUSE_API_KEY`; optional `GEMINI_MODEL` | Interactive v1.3.0 loop: composites, role/name filtering, filing helpers, turn-by-turn pipelines, disambiguation, record truncation limits. |
| `claude_uk_companies_house_handler.py` | `finance/uk_companies_house_handler` | Claude | `[finance_uk_companies_house_handler]`, `[claude]` | `ANTHROPIC_API_KEY`, `COMPANIES_HOUSE_API_KEY`; optional `ANTHROPIC_MODEL` (default `claude-haiku-4-5-20251001`) | Interactive v1.3.0 loop with multi-turn tool chaining, role/name filters, and disambiguation hints. |
| `uk_companies_house_handler_demo.py` | `finance/uk_companies_house_handler` | Local execute | `[finance_uk_companies_house_handler]` | None | Mocked v1.3.0 flows: composites, role/name filtering, filing helpers, turn-by-turn run_pipeline, disambiguation resume, record truncation limits. |
...
| `gemini_gmail_minimal.py` | `office/gmail_handler` | Gemini | `[office_gmail_handler]`, `[gemini]` | `GOOGLE_API_KEY`, `GMAIL_ADDRESS`, `GMAIL_APP_PASSWORD` | Minimal interactive loop — Gemini tool calling wired to Gmail (README quick start). |
| `pay_and_notify_demo.py` | `office/gmail_handler`, `defi/evm_tx_handler` | Local execute | `[office_gmail_handler]`, `[defi_evm_tx_handler]` | Live: `.env` mail + wallet + `BASE_RPC_URL`, `skillware addressbook init`, `skillware evm init`. Smoke: `PAY_NOTIFY_DEMO=1` | Reproducible `SkillContext` chain: resolve → transfer → email receipt. Edit RECIPE constants at top of script. |

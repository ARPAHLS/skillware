...
| `business_diagnostic_demo.py` | `monitoring/business_diagnostic` | Local execute | `[monitoring_business_diagnostic]` | None | Offline scenario ledger lifecycle: adjudicate the in-bundle framework against an observation, calibrate against a resolved outcome, calibrate with a marks history (Brier trend), the honest state on the adjudication date, and two fail-closed contract errors. |

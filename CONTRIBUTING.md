...
- `short_description` — optional one-line routing summary (target <= 120 chars; hard cap 160 after framework truncation) shown in `skillware list`, `SkillContext(mode="brief")` registry lines, and host tool pickers. Write it for an agent choosing among ~20 tools: state what the skill does (verb + object) and when to use it (start with "Use when..." or embed a trigger such as "before..."), disambiguate it from sibling skills, and drop implementation noise. Full Directives are often not injected until `prepare()`, so this line is frequently the only skill-specific routing signal at selection time — see [Skill chaining](docs/usage/skill_chaining.md) for how brief lines differ from Directives.

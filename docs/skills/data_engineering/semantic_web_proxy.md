...
<!-- skill-doc-meta:end -->
<!-- skill-intent:begin -->
**Solves:** Convert live pages or raw HTML into token-efficient Markdown, text, or JSON behind an SSRF guard.
**Works with:** Gemini, Claude, OpenAI, DeepSeek, Ollama, and Bedrock-compatible host loops.
**Runtime:** Network fetch for live URLs; no API key. Offline HTML path available.
<!-- skill-intent:end -->
...
- **Permission is not checked.** This skill does not read robots.txt. Use [`compliance/tos_evaluator`](../compliance/tos_evaluator.md) first when a site's terms are in question.

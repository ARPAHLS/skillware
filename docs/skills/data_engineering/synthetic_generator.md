...
<!-- skill-doc-meta:end -->
<!-- skill-intent:begin -->
**Solves:** Generate high-entropy synthetic JSONL for model fine-tuning without mode collapse.
**Works with:** Gemini, Claude, OpenAI, DeepSeek, Ollama, and Bedrock-compatible host loops.
**Runtime:** Offline templates, or `GOOGLE_API_KEY` / `ANTHROPIC_API_KEY` when a cloud generator backend is selected.
<!-- skill-intent:end -->
...
[Skill Library](../README.md) · [Data Engineering](README.md) · [Testing](../../TESTING.md)
...
response = client.messages.create(
    ...
    tools=tools,
    messages=[{"role": "user", "content": "Generate five diverse medical coding dispute samples."}],
)
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {"role": "user", "content": "Generate five diverse medical coding dispute samples."},
    ],
    tools=[tool],
)
...
prompt = (
    ...
    f"Instructions:\n{bundle['instructions']}\n"
    f"User: Generate five diverse medical coding dispute samples."
)
print(prompt)
result = skill.execute({
    "domain": "medical_coding_disputes",
...
| [`525ecd0`](https://github.com/ARPAHLS/skillware/commit/525ecd01967080bc9631a04395ade9b80c3403d4) | docs: migrate Gemini defaults to 3.5 Flash / Flash-Lite — default generator model (#265) (#345) | 9 Sep 2026 | `0.1.1` | [@rosspeili](https://github.com/rosspeili) |

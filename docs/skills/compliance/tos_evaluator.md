...
<!-- skill-doc-meta:end -->
<!-- skill-intent:begin -->
**Solves:** Decide whether an intended automated web action appears permitted by robots.txt and site legal pages.
**Works with:** Gemini, Claude, OpenAI, DeepSeek, Ollama, and Bedrock-compatible host loops.
**Runtime:** Network fetch for live pages; optional `GOOGLE_API_KEY` when the LLM evaluator is enabled.
<!-- skill-intent:end -->

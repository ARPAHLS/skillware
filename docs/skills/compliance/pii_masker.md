...
<!-- skill-doc-meta:end -->
<!-- skill-intent:begin -->
**Solves:** Detect and redact personally identifiable information locally before text reaches an external model.
**Works with:** Gemini, Claude, OpenAI, DeepSeek, Ollama, and Bedrock-compatible host loops.
**Runtime:** Offline (`micro-f1-mask`); first run may download the local model.
<!-- skill-intent:end -->

**Recommended install:** `pip install "skillware[compliance_pii_masker]"`. See [Install extras](../../usage/install_extras.md).
**Category**: Compliance

[Skill Library](../README.md) · [Compliance](README.md) · [Testing](../../TESTING.md)
...
response = client.messages.create(
    ...
    tools=tools,
    messages=[{"role": "user", "content": "Mask PII in this customer support transcript."}],
)
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {"role": "user", "content": "Mask PII in this customer support transcript."},
    ],
    tools=[tool],
)
...
prompt = (
    ...
    f"Instructions:\n{bundle['instructions']}\n"
    f"User: Mask PII in this customer support transcript."
)
print(prompt)
result = skill.execute({
    "text": "Hello John Doe, your wallet 0xabc123 has been verified.",

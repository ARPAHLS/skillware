...
<!-- skill-doc-meta:end -->
<!-- skill-intent:begin -->
**Solves:** Select query-relevant document spans with local embeddings so host agents stay inside the context window.
**Works with:** Gemini, Claude, OpenAI, DeepSeek, Ollama, and Bedrock-compatible host loops.
**Runtime:** Offline (`fastembed`).
<!-- skill-intent:end -->
...
For untrusted documents, chain [`security/prompt_injection_firewall`](../security/prompt_injection_firewall.md) first — see [Skill chaining](../../usage/skill_chaining.md).

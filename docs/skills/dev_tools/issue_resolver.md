...
<!-- skill-doc-meta:end -->
<!-- skill-intent:begin -->
**Solves:** Turn a GitHub issue URL into a staged, gated resolution workflow with optional repository context.
**Works with:** Gemini, Claude, OpenAI, DeepSeek, Ollama, and Bedrock-compatible host loops.
**Runtime:** Optional `GITHUB_TOKEN` (required for private repos and higher rate limits).
<!-- skill-intent:end -->

**Recommended install:** `pip install "skillware[dev_tools_issue_resolver]"`. See [Install extras](../../usage/install_extras.md).
[Skill Library](../README.md) · [Dev Tools](README.md) · [Testing](../../TESTING.md)
...
response = client.messages.create(
    ...
    tools=tools,
    messages=[{"role": "user", "content": "Analyze https://github.com/owner/repo/issues/42 and propose a fix plan."}],
)
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {"role": "user", "content": "Analyze https://github.com/owner/repo/issues/42 and propose a fix plan."},
    ],
    tools=[tool],
)
...
prompt = (
    ...
    f"Instructions:\n{bundle['instructions']}\n"
    f"User: Analyze https://github.com/owner/repo/issues/42 and propose a fix plan."
)
print(prompt)
result = skill.execute({
    "issue_url": "https://github.com/owner/repo/issues/42",
